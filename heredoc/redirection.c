...
int check_redirect_in(t_cmd **tmp, int flag[])
{
    ...
    while ((*tmp)->arg->init_cmd[i])
    {
        ...
        if ((*tmp)->arg->init_cmd[i] == '<' && flag[0] == 0 && flag[1] == 0)
        {
            str = get_heredoc((*tmp)->arg->init_cmd, i + 1, &len);
            str = remove_quote(str);
            //printf("str: %s ilen: %d, i : %d", str, len, i);
            (*tmp)->arg->init_cmd = delete_char(&((*tmp)->arg->init_cmd), i, len);
            i = i - 1;
    ...
    while (tmp)
    {
        ...
        while (tmp->arg->init_cmd[i])
        {
            ...
            if (!ft_strncmp(tmp->arg->init_cmd + i, ">>", 2) && flag[0] == 0 && flag[1] == 0)
            {
                ...
                printf("str: %s len: %d, i : %d", tmp->arg->init_cmd, len, i);
                i = i - 1;
                tmp->outfile = open(str, O_RDWR | O_CREAT | O_TRUNC, 0644);
                if (tmp->outfile < 0)
                    return (1);
